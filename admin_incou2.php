...
if (is_uploaded_file($_FILES['myfile']['tmp_name'])) {
    ...
    for($j=2;$j<=$highestRow;$j++)  
    {
        ...
        $e = $objPHPExcel->getActiveSheet()->getCell("E".$j)->getValue();//获取D(教师号)列的值
        $f = $objPHPExcel->getActiveSheet()->getCell("F".$j)->getValue();//获取D(教师号)列的值
        $g = $objPHPExcel->getActiveSheet()->getCell("G".$j)->getValue();//获取D(教师号)列的值
        $sql = "insert into course values('$a','$b','$c','$d','$e','$f','$g')";
        $result = mysql_query($sql,$conn);
    }
    if ($result) {
            echo "<script>alert('添加成功！');window.location.href='./admin_addcd.php';</script>";
